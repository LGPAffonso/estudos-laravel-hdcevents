@extends('layouts.main')

@section('title', 'Dashboard')

@section('content')
<!-- <div id="search-container" class="col-md-12">
    <h1>Buscar meus eventos</h1>
    <form action="/dashboard" method="GET">
        <input type="text" id="search" name="search" class="form-control" placeholder="procurar....">
        <input type="submit" value="Enviar">
    </form>
</div> -->
<div class="col-md-10 offset-md-1 dashboard-title-container">
    <h1>Meus eventos</h1>
    <!-- @if($search)
    <h2>Buscando por: {{$search}}</h2>
    @endif -->
</div>

<div class="col-md-10 offset-md-1 dashboard-events-container">
    @if(count($events)>0)
    <table class="table">
        <thead>
            <tr>
                <th scope="col">#</th>
                <th scope="col">Nome</th>
                <th scope="col">Participantes</th>
                <th scope="col">Ações</th>
            </tr>
        </thead>

        <tbody>
            @foreach($events as $event)
            <tr>
                <td scope="row">{{$loop->index + 1}}</td>
                <td><a href="/events/{{$event->id}}">{{$event->titulo}}</a></td>
                <td>{{ count($event->users) }}</td>
                <td><a href="/events/edit/{{$event->id}}" class="btn  btn-info edit btn"> <ion-icon name='create-outline'></ion-icon> Editar</a>
                    <!-- <a href="#">Deletar</a></td> -->
                    <form action="/events/{{$event->id}}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger delete-btn"><ion-icon name="trash-outline"></ion-icon>Delete</button>

                    </form>
            </tr>
            @endforeach


        </tbody>
    </table>
    @else
    <p>Ainda não há eventos disponíveis</p>
    <p><a href="/events/create">criar evento</a></p>
    @endif
</div>
<div class="col-md-10 offset-md-1 dashboard-title-container">
    <h1>Eventos que estou participando</h1>
    <!-- @if($search)
    <h2>Buscando por: {{$search}}</h2>
    @endif -->
</div>
<div class="col-md-10 offset-md-1 dashboard-events-container">

    @if(count($events)>0)
    <table class="table">
        <thead>
            <tr>
                <th scope="col">#</th>
                <th scope="col">Nome</th>
                <th scope="col">Participantes</th>
                <th scope="col">Ações</th>
            </tr>
        </thead>

        <tbody>
            @foreach($eventsasparticipant as $event)
            <tr>
                <td scope="row">{{$loop->index + 1}}</td>
                <td><a href="/events/{{$event->id}}">{{$event->titulo}}</a></td>
                <td>{{ count($event->users) }}</td>
                <td>
                    <form action="/events/leave/{{$event->id}}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger delete-btn"><ion-icon name="exit-outline"></ion-icon>Sair</button>

                    </form>
                </td>
            </tr>
            @endforeach


        </tbody>
    </table>
    @else
    <p>Ainda não está participando de nenhum evento</p>
    <p><a href="/">Ver todos os eventos</a></p>
    @endif
</div>




@endsection